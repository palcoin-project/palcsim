...
import (
	...
	"github.com/palcoin-project/palcd/btcjson"
	rpc "github.com/palcoin-project/palcd/rpcclient"
	"github.com/palcoin-project/palcd/wire"
	"github.com/palcoin-project/palcutil"
)

// minFee is the minimum tx fee that can be paid
const minFee palcutil.Amount = 1e4 // 0.0001 BTC
...
type Actor struct {
	*Node
	quit             chan struct{}
	wg               sync.WaitGroup
	ownedAddresses   []palcutil.Address
	utxoQueue        *utxoQueue
	miningAddr       chan palcutil.Address
...
func NewActor(node *Node, port uint16) (*Actor, error) {
	...
	a := Actor{
		...
		ownedAddresses:   make([]palcutil.Address, *maxAddresses),
		miningAddr:       make(chan palcutil.Address),
...
func (a *Actor) simulateTx(downstream <-chan palcutil.Address, txpool chan<- struct{}) {
	defer a.wg.Done()
	...
	for {
		...
		case utxo := <-a.utxoQueue.dequeue:
			...
			case addr := <-downstream:
				...
				amounts := map[palcutil.Address]palcutil.Amount{
					addr: amt,
				}
	...
	for {
		...
		case utxo := <-a.utxoQueue.dequeue:
			...
			case split := <-split:
				...
				for i := 0; i <= split; i++ {
					var to palcutil.Address
					var change palcutil.Amount
					...
					if i == split {
						...
						change = palcutil.Amount(rand.Int63n(int64(amt) / 2))
						if change < minFee {
							change = minFee
						}
						amt -= change
...
func (a *Actor) sendRawTransaction(inputs []btcjson.TransactionInput, amounts map[palcutil.Address]palcutil.Amount) error {
