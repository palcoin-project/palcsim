...
type Communication struct {
	wg            sync.WaitGroup
	downstream    chan palcutil.Address
...
func NewCommunication() *Communication {
	return &Communication{
		downstream:    make(chan palcutil.Address, *numActors),
...
func (com *Communication) Start(actors []*Actor, node *Node, txCurve map[int32]*Row) (tpsChan chan float64, tpbChan chan int) {
	...
	miningAddrs := make([]palcutil.Address, *numActors)
		...
		case b, ok := <-com.blockQueue.dequeue:
			...
			next:
				for n, vout := range tx.TxOut {
					...
					if txout.Amount > palcutil.Amount((*maxSplit))*(minFee) {
...
func (com *Communication) getUtxo(tx *wire.MsgTx,
	...
	unspent := TxOut{
		OutPoint: op,
		Amount:   palcutil.Amount(vout.Value),
	}
	return &unspent
}
